...
<div class="well textcenter search-domain" style="margin-top: 25px;"  style="margin-top: 25px;">
    <div class="styled_title">{$LANG.domainbulksearchintro}</div>
    ...
    <div class="textcenter">
        <div class="capatacha-wrap nomargin control-group">
            ...
            <div class="internalpadding">
                <input type="submit" value="{$LANG.checkavailability}" class="message-submit" />
            </div>
        </div>
    </div>
</div>
...
<table class="table table-striped table-framed">
    ...
    <tbody>
{foreach from=$tldpricelist item=tldpricing}
        <tr>
            <td>{$tldpricing.tld}</td>
            <td class="textcenter">{$tldpricing.period}</td>
            <td class="textcenter">{if $tldpricing.register}{$tldpricing.register}{else}{$LANG.domainregnotavailable}{/if}</td>
            <td class="textcenter">{if $tldpricing.transfer}{$tldpricing.transfer}{else}{$LANG.domainregnotavailable}{/if}</td>
            <td class="textcenter">{if $tldpricing.renew}{$tldpricing.renew}{else}{$LANG.domainregnotavailable}{/if}</td>
        </tr>
{/foreach}
    </tbody>
</table>
