...
namespace extensions\Yaml;

use extensions\Yaml\Exception\ParseException;
use extensions\Yaml\Exception\DumpException;
                ...
                switch ($mapping[$i]) {
                    case '[':
                        ...
                        if (!isset($output[$key])) {
                            ...
                            @trigger_error(sprintf('Duplicate key "%s" detected whilst parsing YAML. Silent handling of duplicates in YAML is deprecated since version 3.2 and will throw \extensions\Yaml\Exception\ParseException in 4.0.', $key), E_USER_DEPRECATED);
                        }
                        $done = true;
                        break;
                    case '{':
