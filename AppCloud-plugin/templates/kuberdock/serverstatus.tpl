    ...
    <table class="table custom">
        ...
        <tbody>
            {foreach from=$servers key=num item=server}
                <tr>
                    <td>{$server.name}</td>
                    <td class="textcenter" id="port80_{$num}"><img src="{$BASE_PATH_IMG}/loadingsml.gif" alt="{$LANG.loading}" /></td>
                    <td class="textcenter" id="port21_{$num}"><img src="{$BASE_PATH_IMG}/loadingsml.gif" alt="{$LANG.loading}" /></td>
                    <td class="textcenter" id="port110_{$num}"><img src="{$BASE_PATH_IMG}/loadingsml.gif" alt="{$LANG.loading}" /></td>
                    <td class="textcenter"><a href="{$server.phpinfourl}" target="_blank">{$LANG.serverstatusphpinfo}</a></td>
                    <td class="textcenter" id="load{$num}"><img src="{$BASE_PATH_IMG}/loadingsml.gif" alt="{$LANG.loading}" /></td>
                    <td class="textcenter" id="uptime{$num}"><img src="{$BASE_PATH_IMG}/loadingsml.gif" alt="{$LANG.loading}" /><script> checkPort({$num},80); checkPort({$num},21); checkPort({$num},110); getStats({$num}); </script></td>
                </tr>
            {foreachelse}
                <tr>
